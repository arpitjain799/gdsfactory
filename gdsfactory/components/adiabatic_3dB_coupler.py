...
def adiabatic_coupler(
    ...
    add_ports(
        adiabatic_coupler_comp, points_top_arm, points_bot_arm, port_layer, port_width
    )
...
def add_ports(component, points_top_arm, points_bot_arm, port_layer, port_width):
    """Adds ports to the adiabatic coupler.
    ...
    for ii in range(-1, len(points_bot_arm) - 1):
        ...
        if (
            abs(current_point[0] - next_point[0]) < 1e-5
        ):  # check if neighbouring points have the same x-coord
            points_port_bot.append(
                [current_point[0], (current_point[1] + next_point[1]) / 2]
            )

    points_port_bot = sorted(
        points_port_bot, key=lambda k: k[0]
    )  # sort the points in the ascending x-coord order
    if len(points_port_bot) != 2:
        raise ValueError("Coordinates finding of the ports failed")
    ...
    for ii in range(-1, len(points_top_arm) - 1):
        ...
        if (
            ...
            points_port_top.append(
                [current_point[0], (current_point[1] + next_point[1]) / 2]
            )

    points_port_top = sorted(
        points_port_top, key=lambda k: k[0]
    )  # sort the points in the ascending x-coord order
    if len(points_port_top) != 2:
        raise ValueError("Coordinates finding of the ports failed")

    # create the ports
    component.add_port(
        name="in1",
        center=points_port_bot[0],
        orientation=180,
        layer=port_layer,
        width=port_width,
    )
...
def read_points(file) -> []:
    ...
    indices_to_be_kept = list(set(range(len(cord_bot_wg))) - set(duplicate_indices))
    ...
    indices_to_be_kept = list(set(range(len(cord_top_wg))) - set(duplicate_indices))
...
if __name__ == "__main__":
    ...
    file = (
        ...
        / "adiabatic_coupler_files"
        / "adiabatic_coupler_example.mat"
    )
    c = adiabatic_coupler(file=str(file))
